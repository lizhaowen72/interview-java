...
入库单模块介绍
   1-入库审核，5-入库卸货，10 - 入库质检，15- 入库上架，

出库单模块介绍：
    20-出库审核，25-出库拣货, 30 - 出库复核, 35 - 出库装箱, 40-出库发货

中间通过sku（库存保有单位）相连


利用设计模式优化代码(观察者模式),确保接口访问的幂等性,优化出入库单请求响应参数。
...
36.BIO NIO AIO的三者区别？
    ...
    AIO为啥是异步非阻塞？就是说通过AIO发起个文件IO操作之后，你立马就返回可以干别的事儿了，接下来你也不用管了，操作系统自己干完了
